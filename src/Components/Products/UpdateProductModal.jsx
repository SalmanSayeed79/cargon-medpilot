...
export default function UpdateProductModal(props) {
  ...
  const [type,setType]=useState(props.data.variant)
  const [intensity,setIntensity]=useState(props.data.intensity)
  const [brand,setBrand]=useState(props.data.brand)
  const [image,setImage]=useState(props.data.image)
  const [factoryId,setFactoryId]=useState(props.data.factoryId)
  const [category,setCategory]=useState(props.data.category)
  const [chemicalName,setChemicalName]=useState(props.data.chemicalName)
  const [price,setPrice]=useState(props.data.price)
  const [bestBefore,setBestBefore]=useState(props.data.bestBefore)

  const [indications,setIndications]=useState(props.data.indications)
  const [pharmacology,setPharmacology]=useState(props.data.pharmacology)
  const [dosage,setDosage]=useState(props.data.dosage)
  const [adminstration,setAdminstration]=useState(props.data.adminstration)
  const [interaction,setInteraction]=useState(props.data.interaction)
  const [sideEffect,setSideEffect]=useState(props.data.side_effects)
  const [precautions,setPrecautions]=useState(props.data.precautions)
  ...
  const formURL=`${baseURL}/product/update/${params.id}`
  const submitForm =()=>{
    axios.put(formURL,{
        name,
        "variant":type,
  ...
  return (
    <Modal open={props.open} sx={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}>
      <Paper elevation={6} sx={{position:"relative",width:"80vw",minHeight:"80vh",overflowY:"scroll",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}>
          ...
          <Box sx={{margin:"2vh 0"}}>
              <Typography variant="h5" color="primary.main" >Update product</Typography>
          </Box>
